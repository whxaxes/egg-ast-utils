...
class SimpleNode {
  ...
  find(key, strict) {
    ...
    function traverse(key, node) {
      ...
      const newKey = utils.getValue(list.shift());
      if (strict) {
        traverse(newKey, nodeList[nodeList.length - 1]);
      } else {
        nodeList.forEach(item => {
          traverse(newKey, item);
        });
      }
    }

    traverse(utils.getValue(list.shift()), this);
